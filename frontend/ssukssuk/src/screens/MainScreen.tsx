...
function PixelBox({ children, style, innerStyle }: any) {
  return (
    <View style={[styles.pixelBoxContainer, style]}>
      ...
      <View
        pointerEvents="none"
        style={[styles.shadeLeft, { left: -PIXEL, opacity: 0.05, width: PIXEL }]}
      />
      <View
        pointerEvents="none"
        style={[
...
export default function MainScreen() {
  ...
  // ✅ [핵심 수정] fetchHome: 404/400 에러를 "식물 없음"으로 처리
  const fetchHome = useCallback(async () => {
    try {
      ...
      // 서버 로직 상 실패지만 200 OK로 올 경우 대비
      if (!res?.data || res.data.success !== true) {
        setHasPlant(false); // 식물 없음으로 처리
        setIsLoading(false);
        setRefreshing(false);
        return;
      }
      ...
      if (d?.mainPlant && d.mainPlant.plantId) {
        setHasPlant(true);
      ...
      const hsRaw =
        typeof d?.healthScore === 'number'
          ...
          : typeof d?.growthStatus?.percentage === 'number'
            ...
            : 0;
      setHealthScore(clamp(Number(hsRaw) || 0, 0, 100));
      ...
      setWaterStatusText(isOkLike(waterStatus) ? '적정함' : '확인\n필요');
      setNutrientStatusText(isOkLike(nutrientStatus) ? '적정함' : '확인\n필요');
      ...
      if (tNum == null || hNum == null) {
        setTempHumText('-');
      } else {
        setTempHumText(`   ${tNum.toFixed(1)}°C / \n ${hNum.toFixed(0)}%`);
      }
  ...
  const fetchTodayNotifications = useCallback(async () => {
    try {
      ...
      const unreadForBadge = listSeenAt
        ? list.filter(
          n => new Date(n.createdAt).getTime() > new Date(listSeenAt).getTime(),
        )
        : list;
      ...
      const balloonCandidate = balloonSeenAt
        ? list.find(
        n =>
          new Date(n.createdAt).getTime() > new Date(balloonSeenAt).getTime(),
      ) || null
        : list[0] || null;
  ...
  useEffect(() => {
    fetchHome();
    const id = setInterval(fetchHome, 5000); // 5초 주기 폴링
