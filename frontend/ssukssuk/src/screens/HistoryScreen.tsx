...
const SAMPLE_PLANT_IMG = require("../assets/tomato_ex.png");
const PIX = 3;
            ...
            <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
                ...
                <PixelBox hasShadow style={{ padding: 0 }}>
                    <View style={styles.photoWrap}>
                        <Image source={SAMPLE_PLANT_IMG} style={styles.photo} resizeMode="cover" />
                ...
                <PixelBox hasShadow style={styles.chartBox}>
                    <View style={styles.chartHeader}>
                        ...
                        <LegendItem label="Height(cm)" color={ACCENT} />
                    </View>
                    <GrowthLineChart
                        days={dummyDays}
                        values={dummyGrowth}
                ...
                <PixelBox hasShadow style={styles.chartBox}>
                    <View style={styles.monitorHeader}>
                        ...
                        <Text style={[styles.monitorTitle, { color: BORDER }]}>ANOMALY MONITOR</Text>
                    </View>
                    <AnomalyMonitorChart
                        days={dummyDays}
                        data={dummyAnomaly}
...
function PixelBox({ children, style, hasShadow }: any) {
    return (
        <View style={styles.pixelContainer}>
            ...
            <View style={[styles.pixelBox, style]}>
                {children}
            </View>
        </View>
    );
...
function GrowthLineChart({ days, values, width, height }: any) {
    ...
    // 선 포인트
    const points = values.map((v, i) => `${p + i * xStep},${yPos(v)}`).join(" ");

    // 영역 채우기용 Path
    const fillPath = `M${p},${height - p} ` +
        values.map((v, i) => `L${p + i * xStep},${yPos(v)}`).join(" ") +
        ` L${p + (values.length - 1) * xStep},${height - p} Z`;

    return (
        <Svg width={width} height={height}>
            {/* 가이드라인 */}
            {[0, 1, 2, 3, 4].map((i) => (
                <Line key={i} x1={p} y1={p + (i * (height - p * 2)) / 4} x2={width - p} y2={p + (i * (height - p * 2)) / 4} stroke="#F0F0F0" strokeWidth={1} />
            ))}
            ...
            {data.map((d: any, i: number) => {
                ...
                const renderStackedBar = (y: number, h: number, color: string) => h > 0 && (
                    <G>
                        ...
                        {/* 픽셀 하이라이트 */}
                        <Rect x={x + 1} y={y + 1} width={2} height={h - 2} fill="white" fillOpacity={0.2} />
                    </G>
                );
...
const styles = StyleSheet.create({
    ...
    statusBadge: { position: 'absolute', top: 10, left: 10, backgroundColor: BORDER, flexDirection: 'row', alignItems: 'center', padding: 4 },
    ...
    folderIcon: { width: 28, height: 20, marginRight: 15 },
    ...
    monitorTitle: { fontSize: 13, fontFamily: "NeoDunggeunmoPro-Regular" },
