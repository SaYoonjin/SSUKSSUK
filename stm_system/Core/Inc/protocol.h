...
#define EVENT_RECOVERY_DONE 0x03
#define EVENT_SENSOR_FAIL   0x04
#define EVENT_PUMP_FAIL     0x05
...
// 전송 (USART3 사용)
HAL_StatusTypeDef proto_send(uint8_t type, uint8_t subtype, const uint8_t *payload, uint8_t len);

// 자주 쓰는 메시지 헬퍼
HAL_StatusTypeDef proto_send_pong(void);
HAL_StatusTypeDef proto_send_sensor_data(uint16_t temp_x10, uint16_t humi_x10, uint16_t ec, uint16_t water);
